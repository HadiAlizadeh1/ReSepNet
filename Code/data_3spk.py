...
Also for know number of speakers during train we return that as num
"""
...
class AudioDataset3(data.Dataset):
    ...
    def __init__(self, json_dir, batch_size, sample_rate=8000, segment=4.0, cv_maxlen=8.0):
        ...
        """
        super(AudioDataset3, self).__init__()
...
class AudioDataLoader3(data.DataLoader):
    """
    NOTE: just use batchsize=1 here, so drop_last=True makes no sense here.
    """

    def __init__(self, *args, **kwargs):
        super(AudioDataLoader3, self).__init__(*args, **kwargs)
...
def _collate_fn(batch):
    """
    Args:
        batch: list, len(batch) = 1. See AudioDataset3.__getitem__()
    Returns:
    ...
    A = sources_pad[0,2,22:100]
    B = sources_pad[0,1,22:100]
    if torch.equal(A, B):
      num = [2]
    else:
      num = [3]
    ...
    return mixtures_pad, ilens, sources_pad,num
...
def load_mixtures_and_sources(batch):
    ...
    for mix_info, s1_info, s2_info,s3_info in zip(mix_infos, s1_infos, s2_infos,s3_infos):
        ...
        # merge s1 and s2
        s = np.dstack((s1, s2,s3))[0]  # T x C, C = 2
        utt_len = mix.shape[-1]
        if segment_len >= 0:
            # segment
...
if __name__ == "__main__":
    ...
    dataset = AudioDataset3(json_dir, int(batch_size))
    data_loader = AudioDataLoader3(dataset, batch_size=1,
                                  num_workers=4)
